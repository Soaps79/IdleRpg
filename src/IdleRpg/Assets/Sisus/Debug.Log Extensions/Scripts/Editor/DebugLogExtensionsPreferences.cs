...
namespace Sisus.Debugging.Settings
{
	...
	public static class DebugLogExtensionsPreferences
	{
		...
		private const char stringJoinChannelSeparator = ';';
		#else
		private const string stringJoinChannelSeparator = ";"; // string.Join does not support char in older versions
		#endif
		...
		public static void Apply()
		{
			...
			if(PlayerPrefs.HasKey(ChannelColorsKey))
			{
				channelColors.Clear();
				defaultChannelColors = PlayerPrefs.GetString(ChannelColorsKey, "").Split(';').Select(c => "#" + c).ToArray();
		...
		private static void DrawSettingsGUI()
		{
			...
			if(GUILayout.Button("Reset All To Defaults"))
			{
				if(EditorUtility.DisplayDialog("Reset All To Defaults?", "Are you sure you want to reset all personal preferences to default values?\n\nThis will not affect any other users of the project.", "Reset All", "Cancel"))
				{
					...
					PlayerPrefs.DeleteKey("DebugLogExtensions.ChannelColors2");
		...
		private static void DrawChannelsElement(List<string> channels, string prefsKey, Rect rect, int index)
		{
			...
			if(!string.Equals(idWas, setId))
			{
				...
				if(!string.Equals(idWas, setId))
				{
					...
					PlayerPrefs.SetString(prefsKey, string.Join(stringJoinChannelSeparator, channels));
					Apply();
				}
			}
		}
		...
		private static void DrawChannelColor(Rect rect, int index, bool isActive, bool isFocused)
		{
			...
			if(colorWas != setColor)
			{
				...
				if(count > 0)
				{
					sb.Append(ColorUtility.ToHtmlStringRGB(channelColors[0]));
				...
				PlayerPrefs.SetString(ChannelColorsKey, sb.ToString());
				Apply();
			}
		}
		...
		private static void RemoveChannelColor(ReorderableList list)
		{
			...
			if(setCount == 0)
			{
				PlayerPrefs.DeleteKey(ChannelColorsKey);
				channelColors.Clear();
				return;
			}
